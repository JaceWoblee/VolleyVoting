...
export default function VotePage() {
  ...
  const onLogin = async (e: React.FormEvent) => {
    ...
    if (isNaN(parsedShirt)) {
      return alert("Please enter a valid shirt number.");
    }
  ...
  return (
    <main className="min-h-screen bg-slate-100 py-8 px-4 text-slate-900">
      ...
      <div className="max-w-md mx-auto space-y-6">
        ...
        {/* CARD 2: OPTIONAL FEEDBACK (Now wrapped in login check and separate container) */}
        {loggedInName && (
          <div className="bg-white rounded-2xl shadow-xl p-6 border border-slate-200 animate-in fade-in slide-in-from-bottom-4">
            <h2 className="text-lg font-bold text-slate-800">Nachricht an den Coach 📬</h2>
            <p className="text-xs text-slate-500 mb-4 italic">Optional: Feedback zum Spiel oder Training?</p>
            ...
            <form onSubmit={handleSendFeedback} className="space-y-3">
              ...
              <label className="flex items-center gap-2 cursor-pointer group">
                ...
                <span className="text-xs text-slate-600 font-medium group-hover:text-slate-900 transition-colors">
                  Anonym senden? (Coach sieht deinen Namen nicht)
                </span>
              </label>
